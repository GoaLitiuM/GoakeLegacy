	...
	if (ttype == PTI_CUBE)
	{
			...
			for (i = 0; i < nummips; i++)
			{
				...
				for (j = 0; j < min(6, mips->mip[i].depth); j++)
				{
					if (gl_config.formatinfo[encoding].type)
						qglTexSubImage2D			(cubeface[j], i, 0, 0, mips->mip[i].width, mips->mip[i].height, gl_config.formatinfo[encoding].format, gl_config.formatinfo[encoding].type, (void*)((char*)mips->mip[i].data + sz*j));
					else
						qglCompressedTexSubImage2D	(cubeface[j], i, 0, 0, mips->mip[i].width, mips->mip[i].height,	ifmt,																	sz, (void*)((char*)mips->mip[i].data + sz*j));
				}
			}
		}
		else
		{
			for (i = 0; i < nummips; i++)
			{
				...
				{
					if (gl_config.formatinfo[encoding].type)
						qglTexImage2D			(cubeface[j], i, ifmt, mips->mip[i].width, mips->mip[i].height, 0, gl_config.formatinfo[encoding].format, gl_config.formatinfo[encoding].type, (void*)((char*)mips->mip[i].data + sz*j));
					else
						qglCompressedTexImage2D	(cubeface[j], i, ifmt, mips->mip[i].width, mips->mip[i].height,	0,                                                                         sz, (void*)((char*)mips->mip[i].data + sz*j));
				}
			}
		}
		...
		if (out.encoding)
		{
			for (i = 0; i < nummips; i++)
			{
				...
				{	//gl's cubemaps are annoying
					...
					out.mip[i].depth = 6;
					for (j = 0; j < 6; j++)
						qglGetCompressedTexImage(targ, j, (void*)((char*)out.mip[i].data + csize*j));
				}
				else
				{
